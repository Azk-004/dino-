...
// ---------------- Sky dome (gradient + stars) ----------------
  ...
  function update(progress, activeIndex) {
    ...
    renderer.toneMappingExposure = THREE.MathUtils.lerp(renderer.toneMappingExposure, cur.exp * 0.88, Math.min(1, dt * 2));
    // Lumières du ciel : suivent la palette du moment (luminosité globale adoucie)
    ambientLight.color.copy(cur.amb);
    ambientLight.intensity = 0.6 * (1 - n) + 0.35 * n;
    hemiLight.color.copy(cur.hs);
    hemiLight.groundColor.copy(cur.hg);
    hemiLight.intensity = 0.4 * (1 - n) + 0.35 * n;
    sunLight.color.copy(cur.sun);
    sunLight.intensity = 1.85 * cur.sunI * (1 - n) + 0.25 * n;
    ...
    panels.forEach((pl, i) => {
      ...
      if (pl.light) {
        ...
        pl.light.intensity = THREE.MathUtils.lerp(pl.light.intensity, targetLight + n * 0.6, lerpRate);
      }
      pl.group.position.y = THREE.MathUtils.lerp(pl.group.position.y, isActive ? 0.22 : 0, 0.06);
      pl.beaconMat.emissiveIntensity = (0.16 + Math.sin(time * 2.4 + i) * 0.08) * (1 - n) + (0.9 + Math.sin(time * 2.4 + i) * 0.2) * n;
      ...
      pl.frontMat.emissiveIntensity = THREE.MathUtils.lerp(
        ...
        pl.nightMode ? n * (0.55 + (isActive ? 0.1 : hovered ? 0.14 : near ? 0.05 : 0)) : 0,
        0.1
      );
      // Cadre en bois : léger éclairage nocturne pour garder la silhouette du panneau visible
      pl.frameMat.emissiveIntensity = THREE.MathUtils.lerp(pl.frameMat.emissiveIntensity, n * 0.22, 0.05);
    ...
    for (const k of kiosks) {
      ...
      k.g.userData.sign.material.emissiveIntensity = n * 0.55;
    }

    // Fenêtres des immeubles : s'illuminent la nuit (léger scintillement par immeuble)
    for (let i = 0; i < buildings.length; i++) {
      buildings[i].material.emissiveIntensity = n * (0.6 + Math.sin(time * 1.6 + i * 1.7) * 0.14);
    }
