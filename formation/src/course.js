...
export function initCourse({ onExit, onScrollTo, onQuiz }) {
  ...
  coverEl.innerHTML = `
    ...
    <p class="course-cover-desc">Les contenus sont complétés par des séances d'explication en présentiel - par visioconférence - animées par le formateur, assisté des différents spécialistes. La formation se conclut par la délivrance d'un certificat de la plateforme.</p>
  ...
  STATIONS.forEach((st) => {
    ...
    // Insérer une carte de chapitre (style IBM) avant la première station de chaque chapitre
    if (st.chapter !== lastChapter && ch && !isQuiz) {
      const chapStations = STATIONS.filter((s) => s.chapter === st.chapter);
      const objectivesHtml = ch.objectives
        ? ch.objectives.map((o) => `<li class="chapter-card__objective"><span class="chapter-card__objective-check">\u2713</span>${o}</li>`).join("")
        : "";
      secHtml.push(`
        <div class="chapter-card chapter-card--visible" style="--accent: ${ch.color || 'var(--terracotta)'}">
          <div class="chapter-card__banner">
            <span class="chapter-card__icon">${ch.icon || ''}</span>
            <span class="chapter-card__number">${ch.name}</span>
          </div>
          <div class="chapter-card__body">
            <h2 class="chapter-card__title">${ch.label}</h2>
            <div class="chapter-card__rule"></div>
            <p class="chapter-card__desc">${ch.description || ''}</p>
            ${objectivesHtml ? `<div class="chapter-card__objectives"><span class="chapter-card__objectives-label">Objectifs du chapitre</span><ul class="chapter-card__objectives-list">${objectivesHtml}</ul></div>` : ''}
            <div class="chapter-card__meta">
              <span class="chapter-card__meta-chip">${chapStations.length} le\u00e7on${chapStations.length > 1 ? 's' : ''}</span>
              ${!isQuiz ? '<span class="chapter-card__meta-chip">Quiz de chapitre inclus</span>' : ''}
            </div>
          </div>
          <div class="chapter-card__corner chapter-card__corner--tl"></div>
          <div class="chapter-card__corner chapter-card__corner--br"></div>
        </div>
      `);
